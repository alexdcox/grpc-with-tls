...
$openssl genpkey -algorithm Ed25519 -out certs/dcrctl-client.key

$openssl req \
  -x509 \
  -CA certs/dcrwallet-clients.pem \
  -CAkey certs/dcrwallet-clients-ca.key \
  -key certs/dcrctl-client.key \
  -out certs/dcrctl-client.cert \
  -days 3650 \
  -sha256 \
  -subj "/O=dcrdwallet client/CN=pop-os" \
  -addext "subjectAltName = DNS:localhost, $ips"
